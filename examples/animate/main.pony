...
actor Main
  ...
  new create(env: Env) =>
    _env = env
    var delay_ns: U64 = 500_000_000_000
    try
    ...
    let notify = AnimateDemo.create(this, _nc, delay_ns)
    let timer = Timer(consume notify, 0, 1_000_000)
  ...
  be demo_finished(ret: I32) =>
    ...
    if ret != 0 then
      _env.err.print("animate failed: " + ret.string())
      _env.exitcode(1)
    end
